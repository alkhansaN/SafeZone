...
?>
            ...
            <?php
               ...
               foreach( $items as $rows)
                {
            ?>
  <option value=<?php echo $rows['User_ID'];?>><?php echo $rows['User_Email'];?></option>
 <?php } ?>
</select>
...
$stmt   = $con -> prepare('SELECT * FROM Trash WHERE  (User_ID=?)');
            $stmt   -> execute(array($_SESSION['User_ID']));
          $items    = $stmt -> fetchAll();
?>
          ...
          <div class="item"><a  href="DataBase/restore.php?FileName=<?php echo $rows['FileName']; ?>"> Restore file </a></div>
   
      <div class="item"><a href="DataBase/deleteTrash.php?FileName=<?php echo $rows['FileName']; ?>">remove</a> </div>
    </div>
